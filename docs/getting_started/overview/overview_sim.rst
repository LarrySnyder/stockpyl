...
To specify that a node has an external custoemr, set its ``demand_source`` attribute
...
.. doctest::
	...
	... )
	>>> simulation(network=network, num_periods=4, rand_seed=42, progress_bar=False)
	write_results(network=network, num_periods=5, columns_to_print='minimal', print_cost_summary=False)0.0
