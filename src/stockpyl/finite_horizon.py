...
def finite_horizon_dp_temp(
	...
	*	In the latter case, the list is assumed to contain values for periods
	...
	Optionally, you can provide ``oul_matrix`` and ``x_range``, in which case the
	function will simply calculate the cost of a given OUL matrix.
	...
	demand_sd : float or list
		Demand standard deviation per period. [:math:`\\sigma`]
	discount_factor : float or list, optional
		Discount factor, in :math:`(0,1]`. Default = 1. [:math:`\\gamma`]
	initial_inventory_level : float, optional
		Initial inventory level at the start of period 1. [:math:`x_1`]
	trunc_tol : float, optional
		...
		:math:`P(s_t - D < x_{min})` > ``trunc_tol``, where :math:`x_{min}` is the 
		minimum value of :math:`x` after truncation.
	d_spread : float, optional
		Number of standard deviations around mean to consider for demand truncation.
	s_spread : float, optional
		Number of (demand) standard deviations around :math:`(s,S)` estimates to consider.
	oul_matrix : ndarray, optional
		Matrix of order-up-to levels to use. Same format as output argument ``oul_matrix``.
		If provided, then ``x_range`` must also be provided.
	x_range : list
		Vector of :math:`x`-values to be used in the discretization for ``oul_matrix``. 
	...
	ValueError
		If ``terminal_holding_cost`` < 0 or ``terminal_stockout_cost`` < 0.
	ValueError
		If ``oul_matrix`` is specified and ``x_range`` is not or does not contain all values in ``oul_matrix``.
	...
	**Example**:
	...
	# If oul_matrix is specified, check x_range.
	if oul_matrix is not None:
		# Make sure x_range is also specified and contains all OULs.
		if x_range is None:
			raise ValueError('If oul_matrix is specified, then x_range must also be specified.')
...
#			nv[t] = norm.ppf(alpha[t], mean[t], demand_sd[t])
	...
	while not done:
		...
		order_up_to_levels = [0] * (num_periods+1)
		cost_matrix = np.zeros((num_periods+2, len(x_range)))
		if oul_matrix is None:
			# Remember user's OUL matrix.
			user_oul_matrix = copy.deepcopy(oul_matrix)
		else:
			user_oul_matrix = None
		oul_matrix = np.zeros((num_periods+1, len(x_range)))
		H = np.zeros((num_periods+1, len(x_range)))
...
#		opt_warning = False
		...
		for t in range(num_periods, 0, -1):
			...
			for x in range(x_min, x_max + 1):
				...
				# Did user specify an OUL matrix?
				if user_oul_matrix is not None:
					y_range = [user_oul_matrix[t, x - x_min]]
				else:
