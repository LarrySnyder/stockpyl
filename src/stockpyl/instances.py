...
def load_instance_old(instance_name, filepath=None, initialize_missing_attributes=True,
...
def load_instance(instance_name, filepath=None, initialize_missing_attributes=True,
	...
	# Try to build a SupplyChainNetwork by converting from the dict. This will fail if the
	# instance is a regular dict, in which case we'll just return the dict.
	try:
...
def save_instance_old(instance_name, instance_data, instance_description='', filepath=None, 
	replace=True, create_if_none=True, omit_state_vars=True):
	"""Save an instance to a JSON file. 
